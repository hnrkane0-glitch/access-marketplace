...
## Manual wallet funding and withdrawals

- **Add money** starts a Paystack payment into the platform account.
- A successful webhook marks the top-up as paid and emails the admin at
  `ADMIN_NOTIFICATION_EMAIL`. It does **not** auto-credit the user.
- The admin uses **Admin → Send money** to credit the exact amount to the
  user's ledger. The user sees the credit in their dashboard.
- **Withdraw** collects bank name, bank code, account name and 10-digit
  account number. It creates a pending withdrawal and emails the admin with
  all details. The admin sends the money manually through Paystack and marks
  the request as sent.
- Dashboards explicitly tell users that manual reconciliation can take a few
  minutes.

## Membership plans

Signup now goes to `/plans` before `/dashboard`.

- Starter: `PLN_4fj6ah6f5oo2tlx` — 7-day trial.
- Eternal: `PLN_moo8asxaof1nqjl`.
- Pro: `PLN_w7pnequ8jp3hc5d`.

Paystack does not currently expose a native free-trial field. Starter therefore
uses a small refundable ₦100 card-verification transaction, then creates the
real subscription with a `start_date` seven days later. This is the Paystack
documented tokenization workaround for free trials. See the official Paystack
subscription documentation.

Users can manage/cancel their recurring card subscription from `/settings`.

## Resend

Create a Resend API key and put it only in `.env` locally and Vercel project
environment variables in production:

```text
RESEND_API_KEY=...
RESEND_FROM_EMAIL=Access Marketplace <notifications@yourverifieddomain.com>
ADMIN_NOTIFICATION_EMAIL=hnrkane0@gmail.com
```

Never commit `.env` or the Resend key.

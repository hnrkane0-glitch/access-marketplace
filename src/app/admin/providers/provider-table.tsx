...
import { Ban, CheckCircle2, Loader2, BadgeCheck } from "lucide-react";
...
export default function ProviderTable({ rows: initialRows }: { rows: Row[] }) {
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-white/5 last:border-0">
                <td className="px-4 py-3">
                  <p className="font-medium">{r.displayName || r.name}</p>
                  <p className="text-xs text-white/40">{r.email}</p>
                </td>
                <td className="px-4 py-3">{r.listingCount}</td>
                <td className="px-4 py-3">{naira(r.revenueKobo)}</td>
                <td className="px-4 py-3">{naira(r.availableKobo)}</td>
                <td className="px-4 py-3 text-white/60">
                  {r.commissionOverrideBps != null ? `${(r.commissionOverrideBps / 100).toFixed(1)}% (override)` : "Platform default"}
                </td>
                <td className="px-4 py-3">
                  {r.verificationLevel === "VERIFIED" ? (
                    <span className="inline-flex items-center gap-1 text-xs text-emerald-400"><BadgeCheck size={13} /> Verified</span>
                  ) : (
                    <span className="text-xs text-white/40">{r.verificationLevel}</span>
                  )}
                </td>
                <td className="px-4 py-3">
                  {r.isSuspended ? (
